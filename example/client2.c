...
int main()
{
  ...
  TCPsocket client2rec = SCVCG_connect("127.0.0.1", 8000);
  // get server socket for receive
  TCPsocket client2rece = SCVCG_connect("127.0.0.1", 8000);
  ...
  */
  ...
  if (pressButton == 1)
  {
    SCVCG_2D_start_send(client2rec);
  }
  else
  {
    //be careful, this function closes the socket
    SCVCG_2D_stop_send(client2rec);
  }
  ...
  */
  ...
  if (getData == 1)
  {
    SCVCG_2D_start_receive(client2rece, 128);
  }
  else
  {
    //be careful, this function closes the socket
    SCVCG_2D_stop_receive(client2rece);
  }
